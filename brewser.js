  ...
  var _brewser = {
    _init: function () {
      ...
      function _detectDevice() {
        ...
        _this.device.phone =
          (_this.browser.mobile || _this.device.touch) && deviceWidth < 600;
        _this.device.tablet =
          (_this.browser.mobile || _this.device.touch) &&
          (deviceWidth >= 600 || /(nexus 7|tablet|ipad|kindle)/g.test(ua));
      ...
      function _isBrowserEdge() {
        return /(edg)/g.test(ua);
      }
          ...
          if (key === "msie") {
            ...
            for (var j = 0; j < arr.length; j++) {
              if (arr[j].indexOf(key) > -1) {
                ...
                if (
                  chunk.substring(
                    chunk.indexOf("/") > -1 && chunk.indexOf(".") > -1
                  )
                ) {
                  if (type === "short") {
      ...
      function _detectBrowser() {
        ...
        } else if (_isBrowserEdge()) {
          type = _this.BROWSERS.edge;
          version = _getBrowserVersion("edg", "short");
          fullVersion = _getBrowserVersion("edg");
          _this.browser.edge = true;
        ...
        _this.browser.mobile =
          /(blackberry|silk|mobile|tablet|mini|android|ios|ipod|iphone|ipad)/g.test(
            ua
          );
      }
      ...
      function _hasCSSTransform() {
        return !!(
          ...
          (window
            ...
            window
              .getComputedStyle(document.body)
              .getPropertyValue("-o-transform") ||
            window
              .getComputedStyle(document.body)
              .getPropertyValue("-moz-transform") ||
            window
              .getComputedStyle(document.body)
              .getPropertyValue("-ms-transform") ||
            window
              .getComputedStyle(document.body)
      ...
      function _detectCanvas() {
        ...
        _this.has.webGL =
          !!canvasEl.getContext && !!canvasEl.getContext("webgl");

        canvasEl = _createEl("canvas");
        _this.has.canvas =
          !!canvasEl.getContext &&
          typeof canvasEl.getContext("2d").fillText === "function";
      }

      function _detectVideo() {
        var videoEl = _createEl("video");
        _this.has.video =
          !!videoEl && typeof videoEl.canPlayType !== "undefined";

        if (_this.has.video) {
          _this.has.videoFormats.mp4 =
            videoEl.canPlayType("video/mp4") !== "" ? true : false;
          _this.has.videoFormats.webm =
            videoEl.canPlayType("video/webm") !== "" ? true : false;
          _this.has.videoFormats.ogg =
            videoEl.canPlayType("video/ogg") !== "" ? true : false;
        } else {
      ...
      function _detectAudio() {
        ...
        _this.has.audio =
          !!audioEl && typeof audioEl.canPlayType !== "undefined";
        _this.has.audioApi = !!("Audio" in window);

        if (_this.has.audio) {
          _this.has.audioFormats.mp3 = !!(
            audioEl.canPlayType &&
            audioEl.canPlayType("audio/mpeg;").replace(/no/, "")
          );
          _this.has.audioFormats.ogg = !!(
            audioEl.canPlayType &&
            audioEl.canPlayType('audio/ogg; codecs="vorbis"').replace(/no/, "")
          );
          _this.has.audioFormats.wav = !!(
            audioEl.canPlayType &&
            audioEl.canPlayType('audio/wav; codecs="1"').replace(/no/, "")
          );
          _this.has.audioFormats.mp4 = !!(
            audioEl.canPlayType &&
            audioEl
